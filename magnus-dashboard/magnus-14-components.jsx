...
export function Magnus14Dashboard({ websocket }) {
  const [view, setView] = useState('overview'); // overview, analyze, outcomes, learning, history, detail
  ...
  const fetchLearningStats = async () => {
    try {
      ...
      if (data.success) {
        ...
        throw new Error(data.error || 'Failed to fetch learning stats');
      }
    } catch (err) {
      console.error('Error fetching learning stats:', err);
      setError('Failed to fetch learning stats: ' + err.message);
    }
  };
  ...
  const handleViewProject = async (projectId) => {
    try {
      setError(null);
      const response = await fetch(`/api/magnus14/projects/${projectId}`);
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}`);
      }
      const data = await response.json();
      if (data.success) {
        setSelectedProjectDetail(data.data);
        setView('detail');
      } else {
  ...
  return (
    <div className="magnus14-dashboard">
      ...
      <div className="magnus14-content">
        ...
        {view === 'detail' && selectedProjectDetail && (
          <ProjectDetailView
...
function Magnus14Overview({ projects, status, accuracy, learning, onAnalyze, onRefresh }) {
  ...
  const radarData = firstProject?.engines ? {
    ...
    domain: { confidence: firstProject.engines.domain?.confidence || 0.72 },
    poc: { confidence: firstProject.engines.poc?.confidence || 0.58 },
    integration: { confidence: firstProject.engines.integration?.confidence || 0.75 },
    sideProject: { confidence: firstProject.engines.sideProject?.confidence || 0.62 },
    framework: { confidence: firstProject.engines.framework?.confidence || 0.68 }
  } : null;
    ...
    <div className="analysis-form-14">
      <div className="form-container-14">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group-14">
            <label htmlFor="projectPath">Project Folder Path (Optional)</label>
            <input
              type="text"
              id="projectPath"
              name="projectPath"
            ...
            />
            <small className="form-hint-14">Local or network path to the project directory for analysis</small>
          </div>
...
function OutcomeRecordingView({ projects, onSuccess, onError }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.actualSpiralCount || !formData.totalDurationMonths) {
      onError('Please fill in spiral count and duration');
      return;
    }
    ...
    try {
      const response = await fetch(`/api/magnus14/outcomes/${selectedProject.projectId}`, {
        ...
        body: JSON.stringify({
          ...
          actualIntegrationComplexity: formData.actualIntegrationComplexity ? parseInt(formData.actualIntegrationComplexity) : undefined,
          actualClarityTime: formData.actualClarityTime || undefined,
          actualBreakthroughSession: formData.actualBreakthroughSession ? parseInt(formData.actualBreakthroughSession) : undefined,
          breakthroughWasCorrect: formData.breakthroughWasCorrect,
          actualDomainBlocker: formData.actualDomainBlocker,
          finalClarity: formData.finalClarity ? parseInt(formData.finalClarity) : undefined
        })
      });
        ...
        <form onSubmit={handleSubmit}>
          ...
          {selectedProject && (
            <>
              ...
              <div className="form-section-header-14">
                <h3>📊 Actual Results (Required)</h3>
              </div>
              ...
              <div className="form-section-header-14">
                <h3>🔧 Advanced Metrics (Optional)</h3>
              </div>

              <div className="form-row-14">
                <div className="form-group-14">
                  <label htmlFor="actualIntegrationComplexity">Integration Complexity (1-10)</label>
                  <input
                    type="number"
                    id="actualIntegrationComplexity"
                    name="actualIntegrationComplexity"
                    value={formData.actualIntegrationComplexity}
                    onChange={handleChange}
                    placeholder="e.g., 8"
                    min="1"
                    max="10"
                ...
                <div className="form-group-14">
                  <label htmlFor="finalClarity">Final Clarity (0-100%)</label>
                  <input
                    type="number"
                    id="finalClarity"
                    name="finalClarity"
                    value={formData.finalClarity}
                    onChange={handleChange}
                    placeholder="e.g., 95"
                    min="0"
                    max="100"
              ...
              <div className="form-row-14">
                <div className="form-group-14">
                  <label htmlFor="actualClarityTime">Time to Clarity</label>
                  <input
                    type="text"
                    id="actualClarityTime"
                    name="actualClarityTime"
                    value={formData.actualClarityTime}
                    onChange={handleChange}
                ...
                <div className="form-group-14">
                  <label htmlFor="actualBreakthroughSession">Breakthrough in Session #</label>
                  <input
                    type="number"
                    id="actualBreakthroughSession"
                    name="actualBreakthroughSession"
                    value={formData.actualBreakthroughSession}
                    onChange={handleChange}
                    placeholder="e.g., 2"
                    min="1"
                  />
              ...
              <div className="form-row-14">
                <div className="form-group-14">
                  <label htmlFor="actualDomainBlocker">Real Blocker Type</label>
                  <select
                    id="actualDomainBlocker"
                    name="actualDomainBlocker"
                    value={formData.actualDomainBlocker}
                    onChange={handleChange}
                  >
                    <option value="technical">Technical</option>
                    <option value="domain">Domain Knowledge</option>
                    <option value="both">Both Technical & Domain</option>
                  </select>
                </div>

                <div className="form-group-14 checkbox-group-14">
                  <label>
                    <input
                      type="checkbox"
                      name="breakthroughWasCorrect"
                      checked={formData.breakthroughWasCorrect}
                      onChange={handleChange}
                    />
                    <span>Breakthrough prediction was correct</span>
                  </label>
                </div>
              </div>
...
function ProjectDetailView({ project, onBack }) {
  if (!project) {
    return <div className="empty-state-14"><p>Project not found</p></div>;
  }

  return (
    <div className="detail-view-14">
      <div className="detail-header-14">
        <button className="btn-back-14" onClick={onBack}>← Back to History</button>
        <h2>{project.projectName}</h2>
        <span className="domain-badge-14">{project.domain}</span>
      </div>

      <div className="detail-grid-14">
        {/* Project Info */}
        <div className="detail-card-14">
          <h3>📋 Project Information</h3>
          <div className="detail-item-14">
          ...
          <div className="detail-item-14">
            ...
            <span className="value">{new Date(project.timestamp).toLocaleString()}</span>
          </div>
          <div className="detail-item-14">
            <span className="label">Project ID:</span>
            <span className="value code-14">{project.projectId}</span>
          </div>
        </div>
        ...
        {project.analysis && (
          <div className="detail-card-14">
            ...
            <div className="detail-item-14">
              <span className="label">Expected Spirals:</span>
              <span className="value">{project.analysis.expectedSpirals || 'N/A'}</span>
            </div>
            <div className="detail-item-14">
              <span className="label">Duration (months):</span>
              <span className="value">{project.analysis.durationMonths || 'N/A'}</span>
            </div>
            <div className="detail-item-14">
              <span className="label">Confidence:</span>
              <span className="value">{(project.analysis.confidence * 100 || 0).toFixed(0)}%</span>
            </div>
            <div className="detail-item-14">
              <span className="label">Domain Blocker:</span>
              <span className="value">{project.analysis.domainBlocker || 'N/A'}</span>
            </div>
          </div>
        )}
        ...
        {project.engines && (
          <div className="detail-card-14">
            <h3>⚙️ 6-Engine Scores</h3>
            <div className="engine-scores-14">
              {['spiral', 'domain', 'poc', 'integration', 'sideProject', 'framework'].map(engine => (
                <div key={engine} className="engine-score-item-14">
                  <span className="engine-name-14">{engine.charAt(0).toUpperCase() + engine.slice(1)}</span>
                  <div className="score-bar-14">
                    <div
                      className="score-fill-14"
                      style={{ width: `${(project.engines[engine]?.confidence || 0) * 100}%` }}
                    />
                  </div>
                  <span className="score-value-14">
                    {((project.engines[engine]?.confidence || 0) * 100).toFixed(0)}%
                  </span>
                </div>
              ))}
            </div>
        ...
        {/* Outcome Info (if exists) */}
        {project.outcome && (
          <div className="detail-card-14">
            <h3>📈 Recorded Outcomes</h3>
            <div className="detail-item-14">
              <span className="label">Actual Spirals:</span>
              <span className="value">{project.outcome.actualSpiralCount}</span>
            </div>
            <div className="detail-item-14">
              <span className="label">Actual Duration:</span>
              <span className="value">{project.outcome.totalDurationMonths} months</span>
            </div>
            {project.outcome.actualIntegrationComplexity && (
              <div className="detail-item-14">
                <span className="label">Integration Complexity:</span>
                <span className="value">{project.outcome.actualIntegrationComplexity}/10</span>
              </div>
            )}
            {project.outcome.finalClarity && (
              <div className="detail-item-14">
                <span className="label">Final Clarity:</span>
                <span className="value">{project.outcome.finalClarity}%</span>
              </div>
            )}
            {project.outcome.actualDomainBlocker && (
              <div className="detail-item-14">
                <span className="label">Actual Blocker:</span>
                <span className="value">{project.outcome.actualDomainBlocker}</span>
              </div>
            )}
          </div>
        )}
...
function ProjectHistoryView({ projects, onRefresh, onViewProject }) {
...
function ProjectRow14({ project, onView }) {
  return (
