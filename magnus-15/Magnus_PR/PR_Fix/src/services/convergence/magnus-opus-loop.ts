// src/services/convergence/magnus-opus-loop.ts
...
export class OpusFindingsParser {
	...
	public parse(opusResponse: string): OpusTherapeuticFindings {
		...
		const patternMatch = opusResponse.match(/Patterns Magnus.*?:([\s\S]*?)(?:Insight|Robustness|Fixes)/i)
		if (patternMatch) {
		...
		const insightMatch = opusResponse.match(
			/Insight therapeutique.*?:([\s\S]*?)(?:Score|Fixes|Commentaires)/i,
		)
		if (insightMatch) {
		...
		const fixesMatch = opusResponse.match(/Fixes.*?:([\s\S]*?)(?:Commentaires|$)/i)
		if (fixesMatch) {
		...
		if (commentMatches) {
			findings.inlineComments = commentMatches.map((comment) => {
				const match = comment.match(/\/\/\s*OPUS:\s*([A-Z_]+):\s*(.+?)\s*->\s*pattern:\s*([A-Z_]+)/)
				return match ? { comment: match[2], mappedPattern: match[3] } : { comment }
			})
		}
	 ...
	 */
	private extractFixes(text: string): OpusTherapeuticFindings["suggestedFixes"] {
		const fixes: OpusTherapeuticFindings["suggestedFixes"] = []
		const lines = text.split("\n").filter((l) => l.trim())

		for (const line of lines) {
			const match = line.match(/(.+?)\s*->\s*pattern:\s*([A-Z_]+).*?severity:\s*([A-Z_]+)/i)
			if (match) {
				fixes.push({
					issue: match[1].trim(),
					pattern: match[2],
...
export class MagnusOpusTherapeuticLoop {
	...
	constructor(logger?: ILogger) {
		this.logger = logger || new CompactLogger()
		this.magnusEngine = new MagnusPatternEngine(this.logger)
		this.promptGenerator = new TherapeuticPromptGenerator()
		this.findingsParser = new OpusFindingsParser(this.logger)
	}
	...
	public async executeTherapeuticLoop(
		...
		// === PHASE 1: Magnus pre-analysis ===
		this.logger.debug("Phase 1: Magnus pre-analysis")
		const _initialMagnusResult = this.magnusEngine.detectPatterns(
		...
		const opusPrompt = this.promptGenerator.generatePrompt(mentalProcess, codeReview?.code)
		...
		const enriched = this.enrichMentalProcess(mentalProcess, opusFindings, enrichedMagnusResult)
	 ...
	 */
	private generateMockOpusResponse(_prompt: string): string {
		return `
1. PATTERNS MAGNUS DETECTED:
...
2. INSIGHT THERAPEUTIQUE:
   Vous etes en transition de "chance" vers "competence". La spirale que vous ressentez n'est pas du chaos -- c'est une clarification en cours. Vos assertions et tests montrent que vous externalisez votre incertitude en preuves. C'est la therapie en action.
...
`
	...
	private enrichMentalProcess(
		...
		const allPatterns = Array.from(new Set([...opusFindings.patternsDetected, ...magnusResult.patterns]))
		...
		const harmonyScore =
			positivePatterns.length > 2 ? 0.8 + magnusResult.harmonyScore * 0.2 : magnusResult.harmonyScore

		const therapyPhase = this.determineTherapyPhase(opusFindings.robustnessScore, allPatterns.length)

		const actionPlan = this.generateActionPlan(opusFindings.suggestedFixes, allPatterns)
		...
		return {
			...
			therapyPhase,
			confidence:
				opusFindings.confidence > 0.8 ? "FORT" : opusFindings.confidence > 0.5 ? "MOYEN" : "FAIBLE",
			timestamp: Date.now(),
		}
	}

	private determineTherapyPhase(robustness: number, _patternCount: number): string {
		if (robustness < 50) {
	...
	private generateActionPlan(fixes: OpusTherapeuticFindings["suggestedFixes"], patterns: string[]): string[] {
		const plan: string[] = []
	...
	public getTherapyStatistics(results: EnrichedMentalProcess[]) {
		return {
			...
			averageRobustness: results.reduce((sum, r) => sum + r.robustness, 0) / results.length,
			averageHarmony: results.reduce((sum, r) => sum + r.harmonyScore, 0) / results.length,
